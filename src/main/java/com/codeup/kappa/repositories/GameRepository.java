...
//    Game getGameByGamesApiId(long id);
    ...
    @Query(value = "SELECT * FROM gamerhaven_db.games g WHERE g.id IN :ids", nativeQuery = true)
    List<Game> findGamesByIds(List<Long> ids);

    @Query(value = "SELECT * FROM gamerhaven_db.games g WHERE g.games_api_id = :api_id", nativeQuery = true)
    Game findGameByGamesApiId(long api_id);
//    List<Game> getGameByUserId(long id);

    @Query(value = "SELECT games_api_id FROM gamerhaven_db.games", nativeQuery = true)
